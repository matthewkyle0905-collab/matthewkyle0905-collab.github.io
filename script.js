...
// ============== SLIDESHOW FUNCTIONS (TASK 4) ==============

// Slideshow data using your Google Drive images
const slideshowProducts = [
    {
        id: 'slide-photocard',
        name: 'Photo Cards',
        price: '₱600',
        image: 'https://drive.google.com/uc?export=view&id=1TjeZ9cMDpV5cDO4kKbf2i5LAVRYmZMgB',
    ...
    },
    {
        ...
        name: 'Calendar',
        price: '₱900',
        image: 'https://drive.google.com/uc?export=view&id=1Vk4kCk5H7aBfQNIMfowj-zK5KaGVQT9f',
    ...
    },
    {
        ...
        name: 'Photo Book',
        price: '₱1,500',
        image: 'https://drive.google.com/uc?export=view&id=1Mn4SxXIW1k7fAyOmetReCcbB8GlBNXzM',
    ...
    },
    {
        ...
        name: 'Canvas',
        price: '₱2,400',
        image: 'https://drive.google.com/uc?export=view&id=17kZoqDG1oYd01tNSSf189pWL9F2MeVWY',
    ...
    },
    {
        ...
        name: 'Mouse Pads',
        price: '₱480',
        image: 'https://drive.google.com/uc?export=view&id=18LKI-POXkcITgdWz66VAMmgnZzlABgWz',
    ...
    },
    {
        ...
        name: 'Double Cards',
        price: '₱720',
        image: 'https://drive.google.com/uc?export=view&id=1MlBBcslwfzMmGJ7AuOQJy-UvM8aE5Oou',
...
function initSlideshow() {
    ...
    // Render slides
    track.innerHTML = slideshowProducts.map(product => `
        <div class="slide-card" data-id="${product.id}">
    ...
    // Render dots
    dotsContainer.innerHTML = slideshowProducts.map((_, index) => `
        <button class="dot ${index === 0 ? 'active' : ''}" data-index="${index}" onclick="goToSlide(${index})"></button>
    `).join('');
    ...
    document.getElementById('slideshowPause').addEventListener('click', toggleSlideshowPause);
...
function toggleSlideshowPause() {
    ...
    if (pauseBtn) {
        pauseBtn.textContent = isSlideshowPaused ? '▶️' : '⏸️';
        pauseBtn.setAttribute('aria-label', isSlideshowPaused ? 'Play slideshow' : 'Pause slideshow');
    }
}

// Go to next slide
function nextSlide() {
    const maxSlide = slideshowProducts.length - getSlidesToShow();
...
function prevSlide() {
    ...
    if (currentSlideIndex > 0) {
        ...
        currentSlideIndex = maxSlide; // Loop to end
    }
    updateSlidePosition();
    updateDots();
}
...
function goToSlide(index) {
    const maxSlide = slideshowProducts.length - getSlidesToShow();
    currentSlideIndex = Math.min(index, maxSlide);
    updateSlidePosition();
    updateDots();
}
...
function getSlidesToShow() {
    ...
    if (width <= 480) return slidesToShow.mobile;
    if (width <= 768) return slidesToShow.tablet;
...
// Initialize slideshow when home page is active
function checkAndInitSlideshow() {
    if (document.getElementById('home').classList.contains('active')) {
        // Small delay to ensure DOM is ready
        setTimeout(() => {
            if (!document.getElementById('slidesTrack')?.children.length) {
                initSlideshow();
            }
        }, 100);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Check when navigating to home
    const originalNavigateTo = window.navigateTo;
    window.navigateTo = function(pageId) {
        originalNavigateTo(pageId);
        if (pageId === 'home') {
            checkAndInitSlideshow();
        }
    };
});
